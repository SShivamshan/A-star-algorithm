...
class Network: public Map_creator{
    ...
    private:
        ...
        std::vector<std::pair<int,int>> Dijkstra(Node *_start,Node *_end);
        std::vector<std::pair<int,int>> A_star(Node *_start,Node *_end);
        std::vector<std::pair<int,int>> BFS(Node *_start,Node *_end);
};
